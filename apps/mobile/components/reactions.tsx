...
const Modal = ({
  ...
  return (
    <View
      className={`rounded-3xl h-14 px-3 bg-white border border-dustysky flex flex-row items-center absolute z-20 w-full top-[-56] h-10 overflow-hidden`}
    >
...
const Reactions = (
  { post, groupId }: { post: GroupPost; groupId: string },
  ref: Ref<View>,
) => {
  ...
  const [reactionsGroup, setReactionsGroup] = useState<ReactionsGroup>(
    post.reactionsGroup,
  );
  const [usersReactions, setUsersReactions] = useState<Map<Emoji, string>>(
    new Map(),
  );
  ...
  const addReaction = async (emoji: Emoji) => {
    const result = await groupPostApi.addReaction(post.id, {
      emoji,
      reactionGroupId: post.reactionsGroup.id,
    });
  ...
  return (
    <View ref={ref} className={`flex flex-row gap-2 flex-wrap flex-1`}>
      {isModalOpen && (
        <Modal
          onEmojiSelect={(emoji) => addReactionMutation.mutate(emoji)}
          excluded={[...usersReactions.keys()]}
        />
      )}
      {(reactionsGroup.countPerEmoji ?? []).map(({ emoji, count }) => (
        <Pressable
          ...
          className={`flex h-8 flex-row items-center justify-center badge border-sunbeam self-start ${usersReactions.has(emoji) && "bg-sunbeam/30"}`}
        >
      ...
      <Pressable
        ...
        className={`flex h-8 flex-row items-center justify-center badge border-dustysky self-start`}
      >
