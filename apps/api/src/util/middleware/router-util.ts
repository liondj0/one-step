...
export type RouteMetadata = [path: string, ...handlers: RouteHandler[]];
...
const initMetadata = <Controller extends BaseController>(
  target: Controller,
) => {
...
const decoratorBuilder = (
  path: string,
  method: keyof RouterMetadata,
): MethodDecorator => {
  return (<Controller extends BaseController>(
    target: Controller,
...
export const registerRouter = <Controller extends BaseController>(target: {
  new (): Controller;
}): [string, Hono] => {
